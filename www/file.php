<?php
  ...
  $query = "SELECT * FROM tb_media WHERE media_alias='".c_sql::escape_string($_GET['file'])."' OR media_info->>'filename' LIKE '".c_sql::escape_string($_GET['file'])."' LIMIT 1";
  ...
  $media = json_decode($obj->media_info);
  ...
  if ( stristr($media->mime,"image") && isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) >= $last_modified ) {
    ...
    $fp = fopen($path.$media->filename,"rb");
    $start = 0;
    $end = $media->size-1;
    $length = $media->size;
